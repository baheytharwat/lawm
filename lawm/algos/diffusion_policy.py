...
from lawm.algos.base import ChunkPolicy
from lawm.algos.utils.diffusion_policy_utils.unet_modules import ConditionalUnet1D
from lawm.algos.utils.misc import weight_init

from dreamerv3.utils import Config, apply_size_override, setup_config_for_worldmodel
from dreamerv3.models import WorldModel
...
class DiffusionPolicy(ChunkPolicy):
    def __init__(
        ...
        **kwargs
        ...
        if self.apply_wm:
            ...
            wm_config_path = "dreamerv3/worldmodel_config.yaml"
            with open(wm_config_path, 'r') as f:
                self.wm_config = yaml.safe_load(f)

            size = self.wm_config["defaults"]["model_size"]
            size_key = f"size{size}"
            apply_size_override(self.wm_config, size_key)
            cfg_dict = self.wm_config['defaults'].copy()

            size_overrides = self.wm_config.get(size_key, {})
            cfg_dict = setup_config_for_worldmodel(cfg_dict, size_overrides)
            cfg = Config(**cfg_dict)

            # Create observation space
            image_shape = torch.Size([128, 128, 3])
            obs_space = Dict({'image': Box(low=0, high=1, shape=image_shape, dtype=np.float32)})
            print("Initializing WorldModel...")
            try:
                self.wm = WorldModel(obs_space, cfg)
                print("WorldModel initialized successfully!")
                print(f"Model parameters: {sum(p.numel() for p in self.wm.parameters()):,}")
    ...
    def compute_loss(self, data):
        ...
        if self.apply_wm:
            # Use diffusion model to sample latent actions
            # pred_action = self.networks.diffusion_model.get_action(cond)
            pred_action = self.networks.diffusion_model.get_latent_from_noise(cond)
            wm_input = {
                "image": data["wm_obs"].permute(0, 1, 3, 4, 2),
                "action": pred_action,
                "is_first": data["is_first"],
            }
            loss = self.wm._train(wm_input)
...
class DiffusionModel(nn.Module):
    ...
    def get_latent_from_noise(self, cond, noise=None, timestep=None):
        """
        Differentiable single-step diffusion from noise to latent actions.
        """
        B = cond.shape[0]
        if noise is None:
            noise = torch.randn(B, self.chunk_size, self.action_dim, device=self.device)

        if timestep is None:
            timestep = torch.tensor([self.noise_scheduler.config.num_train_timesteps-1]*B, device=self.device)

        # predict noise residual
        noise_pred = self.net(noise, timestep, global_cond=cond)

        # one-step denoised latent
        latent_action = noise - noise_pred  # or scale by alpha if you want scheduler-aware step
