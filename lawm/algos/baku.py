...
from lawm.algos.utils.gpt import GPT
...
class Baku(ChunkPolicy):
    ...
    def compute_loss(self, data):
        ...
        B, T, D = actions.shape
        if self.apply_wm:
            pred_action = pred_action.loc.view(-1, self.wm_config["defaults"]["batch_length"], self.wm_config["defaults"]["num_actions"])
            wm_input = {"image": data["wm_obs"].permute(0, 1, 3, 4, 2),
                        "action": pred_action,
