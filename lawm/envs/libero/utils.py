...
import lawm.utils.file_utils as FileUtils
import lawm.utils.obs_utils as ObsUtils
from lawm.dataset.sequence_dataset import SequenceDataset
from lawm.dataset.sequence_vl_dataset import SequenceVLDataset, CustomVLADataset
...
def deconstruct_task_name(task_name):
    ...
    if scene_idx == -1: ## This condition for LIBERO Spatial, Object, Goal
        ## Spatial -> KITCHEN
        ## Object -> LIVING_ROOM
        ## Goal -> KITCHEN
        ...
        number = (task_number % 10) + 1 ## To return a number from 1 to 10
        task_number +=1
    else:
...
def build_custom_dataset(data_prefix,
                  suite_name,
                  benchmark_name, 
                  n_tasks,
                  mode, 
    ...
    def parse_filename(filename):
        ...
        task_id = int(splits[0])
        instruction = " ".join(splits[1:])
        return task_id, instruction

    h5_dir = os.path.join(data_prefix, suite_name, benchmark_name)
    h5_files = sorted([f for f in os.listdir(h5_dir) if f.endswith('.hdf5')])[:n_tasks]
    task_info = [parse_filename(f) for f in h5_files]
    descriptions = [desc for _, desc in task_info]

    task_embs_train = []
    for i in tqdm(range(0, len(descriptions), 1000)):
        task_embs_train.extend(get_task_embs(task_embedding_format, descriptions[i:i+1000], train=True))

    datasets = []
    for (filename, (task_id, _)), emb in zip(zip(h5_files, task_info), task_embs_train):
        full_path = os.path.join(h5_dir, filename)
        ds = CustomVLADataset(full_path, task_id=task_id, task_emb=emb["task_emb"], chunk_size=seq_len, apply_wm=apply_wm)
        datasets.append(ds)
