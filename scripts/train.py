...
import lawm.utils.utils as utils
from pyinstrument import Profiler
from lawm.utils.logger import Logger
...
def main(cfg):
    ...
    if checkpoint_path is not None and (not cfg.init_from_pretrained):
        try:
    ...
    elif checkpoint_path is not None and cfg.init_from_pretrained:
        try:
            state_dict = utils.load_checkpoint(checkpoint_path, logger=logger)
            utils.soft_load_state_dict(model, state_dict['model'])
            ...
            logger.warning(f'Failed to load pretrained checkpoint: {str(e)}, starting from scratch')
